...
export default function SettlementsPage() {
  ...
  const [confirmPaidModal, setConfirmPaidModal] = useState<{ id: string; eventTitle: string; netAmount: number } | null>(null);
  ...
  return (
    <div className="space-y-6">
      {/* Title */}
      <div>
        <h1 className="text-2xl font-bold tracking-tight text-slate-800">Settlements (Pencairan Dana)</h1>
        <p className="text-slate-500 text-sm mt-1">
          Kelola pembagian dana hasil penjualan tiket untuk masing-masing organizer setelah event selesai dilaksanakan.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="p-6 bg-white border border-slate-200 rounded-xl flex items-center justify-between shadow-sm">
          <div>
            <span className="text-xs font-bold uppercase tracking-wider text-slate-500">Total Pending Payout</span>
            <div className="text-2xl font-extrabold text-red-600 mt-2">{formatRupiah(totalPendingPayout)}</div>
            <span className="text-xs text-slate-400 block mt-1">Akumulasi dana yang harus ditransfer ke EO</span>
          </div>
          <div className="p-3 bg-red-50 border border-red-200 text-red-650 rounded-lg">
        ...
        <div className="p-6 bg-white border border-slate-200 rounded-xl flex items-center justify-between shadow-sm">
          <div>
            <span className="text-xs font-bold uppercase tracking-wider text-slate-500">Total Biaya Layanan Terkumpul</span>
            <div className="text-2xl font-extrabold text-emerald-600 mt-2">{formatRupiah(totalPlatformFees)}</div>
            <span className="text-xs text-slate-400 block mt-1">Total potongan komisi platform (5%)</span>
          </div>
            ...
            <table className="w-full text-left border-collapse text-sm">
              <thead>
                <tr className="border-b border-slate-200 text-slate-500 font-semibold bg-slate-50">
                  ...
                  <th className="p-4 font-bold text-red-600">Net Payout</th>
              ...
              <tbody className="divide-y divide-slate-100">
                {filteredSettlements.map((set) => (
                  <tr key={set.id} className="hover:bg-slate-50 transition-colors">
                    {/* Event & EO */}
                    <td className="p-4">
                      <div className="font-semibold text-slate-800">{set.eventTitle}</div>
                      <div className="text-xs text-slate-400 mt-0.5">{set.organizerName}</div>
                    </td>
                    {/* Gross */}
                    <td className="p-4 text-slate-650 font-mono text-xs">
                    ...
                    {/* Net Payout */}
                    <td className="p-4 font-bold text-slate-700 font-mono text-xs">
                    ...
                    <td className="p-4 text-right">
                      <div className="flex justify-end items-center gap-2">
                        {set.status !== 'paid' && (
                          <button
                            onClick={() => setConfirmPaidModal({ id: set.id, eventTitle: set.eventTitle, netAmount: set.netAmount })}
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="w-full max-w-md bg-white border border-slate-200 rounded-xl shadow-2xl p-6 relative">
            <div className="flex gap-4">
              <div className="p-3 bg-amber-50 border border-amber-200 text-amber-600 rounded-lg shrink-0">
                <AlertTriangle className="w-6 h-6 animate-pulse" />
              </div>
              <div>
                <h3 className="font-bold text-lg text-slate-900">Konfirmasi Pencairan Dana</h3>
                <p className="text-xs text-slate-500 mt-2 leading-relaxed">
                  Apakah Anda sudah selesai mentransfer dana secara manual sebesar <strong>{formatRupiah(confirmPaidModal.netAmount)}</strong> kepada penyelenggara <strong>{confirmPaidModal.eventTitle}</strong>? 
                </p>
                <p className="text-xs text-slate-400 mt-2 leading-relaxed">
                  Aksi ini tidak dapat dibatalkan dan akan mencatat akun admin Anda sebagai pemroses settlement.
                </p>
              </div>
            </div>
            <div className="mt-6 flex justify-end gap-3 text-xs font-semibold">
              ...
              <button
                ...
                className="px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg transition-colors cursor-pointer shadow-sm"
              >
