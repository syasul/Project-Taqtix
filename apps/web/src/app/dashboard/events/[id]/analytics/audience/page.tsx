      ...
      {isLoading ? (
        ...
        <div className="space-y-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <Card className="bg-slate-900/40 border-slate-850">
              ...
              <CardContent>
                ...
                <p className="text-[10px] text-slate-500 mt-1">Jumlah email/kontak unik terdaftar</p>
              </CardContent>
            </Card>
            ...
            <Card className="bg-slate-900/40 border-slate-850">
              ...
              <CardContent>
                ...
                <p className="text-[10px] text-slate-500 mt-1">Pembeli yang sudah beli event Anda sebelumnya</p>
              </CardContent>
            </Card>
            ...
            <Card className="bg-slate-900/40 border-slate-850">
              ...
              <CardContent>
                ...
                <p className="text-[10px] text-slate-500 mt-1">Rasio loyalitas audiens organizer Anda</p>
              </CardContent>
            </Card>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {/* Retention Donut */}
            <Card className="bg-slate-900/40 border-slate-850 p-6 flex flex-col justify-between">
              <div>
                <CardTitle className="text-sm font-bold text-slate-200">Rasio Buyer Baru vs Lama</CardTitle>
                <CardDescription className="text-xs text-slate-500">Rasio retensi basis audiens acara</CardDescription>
              </div>
                ...
                <div className="flex flex-col md:flex-row items-center gap-6 mt-6">
                  ...
                  <div className="space-y-3 flex-1 w-full">
                    {chartData.map((item, idx) => (
                      <div key={item.name} className="flex justify-between items-center p-3 bg-slate-950/40 border border-slate-900 rounded-xl">
            ...
            <Card className="bg-slate-900/40 border-slate-850 p-6 space-y-4">
              <div>
                <CardTitle className="text-sm font-bold text-slate-200">5 Kota Teratas Pembeli</CardTitle>
                <CardDescription className="text-xs text-slate-500">Sebaran wilayah pembeli tiket terbanyak</CardDescription>
              </div>
              ...
              <div className="space-y-3">
                {audience.topCities.length === 0 ? (
                  ...
                  audience.topCities.map((cityData: any, idx: number) => (
                    <div key={cityData.city} className="flex items-center justify-between p-3.5 bg-slate-950/60 border border-slate-900 rounded-xl">
                      <div className="flex items-center gap-2.5">
                        <span className="h-5 w-5 bg-indigo-500/10 border border-indigo-500/20 text-indigo-400 font-bold rounded-lg flex items-center justify-center text-xs">
                          {idx + 1}
                        </span>
                        <div className="flex items-center gap-1">
                          <MapPin className="h-3.5 w-3.5 text-slate-500" />
                          <span className="text-xs text-slate-200 font-bold capitalize">{cityData.city}</span>
                        </div>
                      </div>
